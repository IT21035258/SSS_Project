                ...
                <select class="form-control" name="election_id" required>
                    ...
                    if ($isAnyElectionAdded > 0) {
                        while ($row = mysqli_fetch_assoc($fetchingElections)) {
                            ...
                            $allowed_candidates = $row['no_of_candidates']; //from election table
                            ...
                            //limitation added for submissions
...
if (isset($_POST['addCandidateBtn'])) {
    ...
    //giving random numbers for phototype
    $candidate_photo = $targeted_folder . rand(111111111, 9999999999) . "_" . rand(111111111, 9999999999) . $_FILES['candidate_photo']['name'];

    //tmp - temporary name
    $candidate_photo_tmp_name = $_FILES['candidate_photo']['tmp_name'];

    //PATH information
    //strtolower - function takes a string as a parameter and converts all uppercase English characters to lowercase.
    $candidate_photo_type = strtolower(pathinfo($candidate_photo, PATHINFO_EXTENSION));

    //allowing photo types excluding .exe type files to prevent from attacks.
    $allowed_types = array("jpg", "png", "jpeg");

    //allowing img sizes
    $image_size = $_FILES['candidate_photo']['size'];

    if ($image_size < 2000000) //1MB = 1000000 bytes
    {
        ...
        if (in_array($candidate_photo_type, $allowed_types)) {
            ...
            if (move_uploaded_file($candidate_photo_tmp_name, $candidate_photo)) {
                ...
                mysqli_query($db, "INSERT INTO candidate_details(election_id, candidate_name, candidate_details, candidate_photo, inserted_by, inserted_on )
                 VALUES('" . $election_id . "','" . $candidate_name . "','" . $candidate_details . "','" . $candidate_photo . "','" . $inserted_by . "','" . $inserted_on . "')") or
                    die(mysqli_error($db));

                echo " <script>location.assign('index.php?AddCandidatePage=1&added=1'); </script>";
            } else {
                echo "<script> location.assign('index.php?AddCandidatePage=1&failed=1'); </script>"; //if failed to upload
            }
        } else {
            echo "<script> location.assign('index.php?AddCandidatePage=1&invalidFile=1'); </script>"; //Invalid File
        }
    } else {
        echo "<script> location.assign('index.php?AddCandidatePage=1&largeFile=1'); </script>"; //Large File
    }



    //photograph logic ending
